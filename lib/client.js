...
try {
    ...
    console.log(" Connected to the RUSH DB");
} catch {
...
let cc = Config.sessionName.replace(/SAVIYA-MD;;;/g, "").replace(/SAVIYA-MD;;;/gi, ""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
...
await updateStatus()
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    console.log("✅ SAVIYA MD Login Successful!");
                    console.log("⬇️  SAVIYA MD Installing External Plugins...");
                    ...
                    for (let i = 0; i < check.length; i++) {
                     ...
                     console.log("😁  SAVIYA External Plugins Installed!");fs.readdirSync(__dirname + "/../commands").forEach((plugin) => {
